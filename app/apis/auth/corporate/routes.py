...
async def corporate_account_verify_email(
        req: VerifyEmail, db: db_dependency, background_tasks: BackgroundTasks
):
    return await CorporateUserAccount.verify_email(req, db, background_tasks)


@corporate_auth_router.post("/corporate-account-generate-otp")
async def corporate_account_otp_for_email_verification(
        req: GenerateOtp, db: db_dependency, background_tasks: BackgroundTasks
):
    return await CorporateUserAccount.otp_for_email_verification(req, db, background_tasks)


@corporate_auth_router.patch("/corporate-account-change_password")
async def corporate_account_change_password(
        req: ChangePassword, db: db_dependency, current_user: auth_dependency, background_tasks: BackgroundTasks
):
    return await CorporateUserAccount.change_account_password(req, db, current_user, background_tasks)


@corporate_auth_router.post("/corporate-account-reset_password")
async def corporate_account_reset_password(req: PasswordResetIn, db: db_dependency, background_tasks: BackgroundTasks):
    return await CorporateUserAccount.account_reset_otp(req, db, background_tasks)


@corporate_auth_router.patch("/corporate-account-confirm-password-reset")
async def corporate_account_confirm_password_reset(
        req: PasswordResetConfirmation, db: db_dependency, background_tasks: BackgroundTasks
):
    return await CorporateUserAccount.account_password_reset(req, db, background_tasks)


@corporate_auth_router.post("/disable-corporate-account", status_code=status.HTTP_200_OK)
async def disable_corporate_account(
